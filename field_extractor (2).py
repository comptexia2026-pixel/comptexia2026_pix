...
class FieldExtractor:
    ...
    def extract_all(self) -> dict:
        ...
        cp_value, cp_source = self._extract_capital_protection(sspa_type)
        wa_value, wa_source = self._extract_worst_or_average(
        ...
        result = {
            ...
            "CAPITAL_PROTECTION": cp_value,
            "MATURITY": self._extract_maturity(),
            "WORST_OR_AVERAGE": wa_value,
            ...
            # Metadata for highlighter (not exported to Excel)
            "_WA_SOURCE": wa_source,
            "_CP_SOURCE": cp_source,
            "_IS_BIL_YES": self._is_bil_yes(),
        }
        ...
        # BIL YES override
        if result.get("_IS_BIL_YES"):
            result["WORST_OR_AVERAGE"] = "W"
            result["_WA_SOURCE"] = "bil_yes"
            result["CAPITAL_PROTECTION"] = "100"
            result["_CP_SOURCE"] = "bil_yes"
    ...
    # =====================================================================
    # BIL YES product detection
    # =====================================================================
    def _is_bil_yes(self) -> bool:
        """Detect if the product is a BIL YES product."""
        return bool(re.search(r"(?i)\bBIL\s+YES\b", self.text[:2000]))
    ...
    # =====================================================================
    def _extract_capital_protection(self, sspa_type: Optional[str] = None) -> tuple:
        """Returns (value, source) where source is 'explicit', 'sspa', or 'none'."""
        # Step 1: label-value pairs
        ...
        if raw:
            ...
            if m:
                return m.group(1).replace(",", "."), "explicit"
        ...
        for p in patterns:
            ...
            if m:
                ...
                if val.lower() in ("none", "kein"):
                    return "0", "explicit"
                try:
                    return str(float(val.replace(",", "."))), "explicit"
                except ValueError:
                    pass
    ...
    # Worst-of / Average
    # =====================================================================
    def _extract_worst_or_average(self, n_underlying_isins: int = 0, n_tickers: int = 0) -> tuple:
        """Returns (value, source) where source explains the decision."""
        # Single underlying = worst-of by definition
        n_underlyings = max(n_underlying_isins, n_tickers)
        if n_underlyings == 1:
            return "W", "single_underlying"
